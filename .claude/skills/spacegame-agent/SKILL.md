...
14. **Persistence** — `SaveableEntity` + `TransformSaveable` + `HealthSaveable`, and
    ...
    extra opt-in.)
15. **Get it into the world**, one of:
...
`EntityInventoryComponent` to the prefab, call `TryAddItem(InventoryItem)` on arrival, and destroy
the world object. Do it inside `Tick`.
...
| Every NPC swings its gun to follow the player's head | `Weapon.UpdateWeaponRotation` aims at `Camera.main` | `EntityEquipmentController` sets `Weapon.ExternallyAimed`; keep `aimHeldItem` on |
