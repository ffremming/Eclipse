...
namespace SpaceGame.EditorTools
{
    ...
    {
        // The controller is read off the player prefab rather than named here: the player's rig
        // has changed once already, and a hardcoded path silently builds the layer onto whichever
        // controller the player used to have.
        ...
        [MenuItem("Tools/Eclipse/Player/Build Upper Body Layer")]
        public static void Build()
        {
            AvatarMask mask = BuildMask();

            AnimatorController controller = PlayerController();
        ...
        /// <summary>
        /// The controller the player character is actually wired to, or null with the reason
        /// logged.
        /// </summary>
        private static AnimatorController PlayerController()
        {
            var player = AssetDatabase.LoadAssetAtPath<GameObject>(PlayerPrefabPath);
            if (player == null)
            {
                Debug.LogError($"PlayerUpperBodySetup: no player prefab at {PlayerPrefabPath}.");
                return null;
            }
            ...
            if (controller == null)
                Debug.LogError($"PlayerUpperBodySetup: {PlayerPrefabPath} has no AnimatorController " +
                               "on its root Animator.");
            return controller;
        }
