// Four bodies arriving at one spawn point, which is the only arrangement this game has.
//
// The world's single SpawnPoint is a child of ShipRV, scattering inside half a metre on the cargo
// bay floor, and every spawn and every respawn is handed the same call on it. The scatter had no
        ...
        public void FourPlayersInARow_AreNotStacked()
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                Assert.IsTrue(manager.TryGetSpawnPoint(out Vector3 position),
                    $"Body {i + 1} of four got no position at all.");
                placed.Add(position);
            }

            // Nothing has a body yet — these four resolved back to back, which is what happens
            // when several spawn coroutines reach this on the same frame. Only the manager's own
            // record of what it has handed out keeps them apart.
