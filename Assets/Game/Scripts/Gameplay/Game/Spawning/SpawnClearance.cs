...
namespace SpaceGame.Gameplay
{
    public static class SpawnClearance
    {
        ...
        ///
        /// The point of asking is not the shelter itself: it is whether a position was authored
        /// against a floor of its own rather than against the open arena.
        /// </summary>
        public static bool IsSheltered(Vector3 position)
        {
            return Physics.Raycast(position + Vector3.up * Skin, Vector3.up, out RaycastHit _,
                                   RoofProbeDistance, ~0, QueryTriggerInteraction.Ignore);
        }

        /// <summary>
        /// Whether <paramref name="position"/> has a floor within <paramref name="maxDrop"/>
        /// beneath it, as opposed to open air.
        /// </summary>
        public static bool StandsOnStructure(Vector3 position, float maxDrop)
        {
            return Physics.Raycast(position + Vector3.up * Skin, Vector3.down, out RaycastHit _,
