...
namespace SpaceGame.Presentation
{
    ...
    /// on while the game is running — which is the point of it. The hotbar edits go through
    /// <see cref="IPlayerInventory"/> rather than the inventory's internals, so they take the same
    /// route a picked-up item does.
    /// </para>
    /// </summary>
    public class DevInventoryUI : MonoBehaviour
    {
        ...
        public void Open()
        {
            ...
            // Redrawn on the inventory's own change event rather than after the click, so the grid
            // updates when the change actually lands — and also when another system moves an item.
            inventory.OnSlotChanged += OnSlotChanged;
            inventory.OnSlotSelected += OnSlotSelected;
