...
//
// Covering only the scene load would not help: by the time the scene "finishes loading" the
// expensive part — the player being placed and the first-frame shader warmup — has not happened
// yet. That is the stutter players actually feel. This waits for the whole chain instead.
//
...
namespace SpaceGame.Presentation
{
    public class LoadingScreenUI : MonoBehaviour
    {
        ...
        /// so loading never reads as a mode the player did not pick.
        /// </summary>
        ...
        // Escape hatch for callers that decide the wait is no longer wanted — a cancelled load, a
        // scene that failed to come up. Safe to call when nothing is showing.
        ...
        private IEnumerator WaitForReady(string sceneToWaitFor)
        {
            ...
            // 2. The player. It is placed by the scene or by the spawner, either of which can land
            //    a frame or two after the scene reports itself loaded.
            yield return WaitUntil(() => GameplayMenuScope.FindLocalPlayer() != null, "player spawn");

            // 3. A camera actually rendering. Until one exists Unity paints its own "no cameras
