...
namespace SpaceGame.Weapons
{
    ...
    public abstract class Weapon : UsableItem
    {
        ...
        /// camera is <c>Camera.main</c> — the player's. The result is every NPC in the world
        /// swinging its barrel to follow the player's head, which looks like the NPCs are watching
        /// you and is nothing of the kind.
        /// </para>
        /// <para>
        ...
        ///
        /// <para>
        /// The camera is the answer only for a weapon the player is holding themselves. A weapon
        /// somebody else is pointing — an NPC's, a turret's — is aimed by whatever is pointing it,
        /// and the two cases below are where this bows out and lets that happen.
        /// </para>
        /// </summary>
        ...
        {
            // What the holder reported wins over anything the weapon can work out for itself: an
            // NPC has no camera, and a weapon in a hand is not where the eyes are.
            if (UseRequest.HasAim)
            {
                return GetSpawnPosition() + UseRequest.AimDirection * 500f;
            }
        ...
        /// <summary>
        /// Where the holder's own camera is pointing, ignoring anything a previous use reported.
        ///
        /// Split out from <see cref="GetAimPoint"/> because the holder has to be able to ask the
        /// question afresh: <see cref="OnRequestUse"/> runs before the new request is assigned, so
        /// a version that consulted it first answered with the aim of the PREVIOUS shot and every
        ...
        protected virtual Vector3 GetFireDirection()
        {
            if (UseRequest.HasAim)
            {
                return UseRequest.AimDirection;
            }

            return GetLocalFireDirection();
        }

        /// <summary>The direction the holder's own camera says. See <see cref="GetLocalAimPoint"/>.</summary>
        ...
        // ─────────── Firing ───────────
        //
        // A shot is two things, and this is where they split: the round that leaves the barrel and
        // the report you hear it leave by.

        /// <summary>
        /// Before the shot: say where it starts and where it points.
        ///
        /// The weapon cannot reconstruct that once the use is running — an NPC holds it by a
        /// socket, and the player's answer is a camera the item does not own.
        /// </summary>
        public override void OnRequestUse(ref UseContext context)
        {
            base.OnRequestUse(ref context);

            // The LOCAL aim, deliberately — this is the moment the holder's own camera is the
            // answer, and GetFireDirection would hand back whatever the previous shot reported.
            Vector3 direction = GetLocalFireDirection();

            context.Origin = GetSpawnPosition();
            context.Aim = direction.sqrMagnitude > 0.0001f
                ? Quaternion.LookRotation(direction)
                : GetFireOrigin().rotation;
        }

        /// <summary>The shot itself: the round that flies, and the hit it registers.</summary>
        protected override void Use()
        {
            TryFire();
        }

        /// <summary>The shot you hear. The round is <see cref="Use"/>'s half.</summary>
