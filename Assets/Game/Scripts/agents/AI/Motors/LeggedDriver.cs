...
//
//   1. A mounted rider, through SteerModule -> IDriveableMotor.ApplyDriveInput.
...
//
// This has to live in Assembly-CSharp: IDriveableMotor and IMovementMotor are declared here, and
...
namespace SpaceGame.Agents
{
    ...
    public abstract class LeggedDriver : MonoBehaviour, IDriveableMotor, IMovementMotor
    {
        [Header("Speeds")]
        ...
        [Tooltip("This machine may travel across its own nose.\n\nOFF is what every machine built " +
                 ...
                 "heading moves to the driver's separate turn action (Turn Action Name). Leave that " +
        ...
        public void ApplyDriveInput(in DriveInput input, float deltaTime)
        {
            ...
            // On a lateral traveller the stick's X means STRAFE, so the heading comes off DriveInput's
        ...
        // riderMotor as (AgentController.Motor as IDriveableMotor) and returns early, so a driver
