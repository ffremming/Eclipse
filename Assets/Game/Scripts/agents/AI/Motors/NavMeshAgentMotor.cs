...
namespace SpaceGame.Agents
{
    ...
    public class NavMeshAgentMotor : MonoBehaviour, IMovementMotor, IJumpMotor, ILeapMotor,
                                     IDriveableMotor, ISelfDrivingMotor, ITeleportAware
    {
        [Header("Navigation")]
        ...
        // Set to Time.frameCount inside ApplyDriveInput so the MoveIntent switch in Tick skips
        // that frame. Arc/cooldown updates still run.
        private int riderDriveFrame = -1;
        // True between a rider's last ApplyDriveInput and our restoration of agent defaults.
        ...
        public void Tick(in MoveIntent intent, float deltaTime)
        {
            ...
            // Rider is driving this frame via ApplyDriveInput — don't re-interpret a MoveIntent.
            if (riderDriveFrame == Time.frameCount)
                return;

            // Rider just released. Restore agent defaults the rider's ApplyDriveInput mutated
        ...
        // writes transform.position from its own internal position every frame: whatever else
        // placed the body is overwritten on the next frame with where the agent thinks it is.
        // Switching the component off is what hands the transform back.
        ...
        public void ApplyDriveInput(in DriveInput input, float deltaTime)
        {
        ...
        ///
        /// The agent's own position is not touched here — Teleport has already warped it, and
        ...
        private void TryReattachToNavMesh(float deltaTime)
        {
            ...
            // Re-attaching would switch the agent back on and hand it a transform something else
            // is writing.
