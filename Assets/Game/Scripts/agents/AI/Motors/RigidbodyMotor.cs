...
namespace SpaceGame.Agents
{
    ...
    public class RigidbodyMotor : MonoBehaviour, IMovementMotor, IJumpMotor, ILeapMotor,
                                  IDriveableMotor, ITeleportAware
    {
        [Header("References")]
        ...
        // Set to Time.frameCount inside ApplyDriveInput so Tick() the same frame skips the
        // MoveIntent-interpretation path (which would decelerate and fight the rider).
        private int riderDriveFrame = -1;

        // Latest rider input, latched in Update and consumed in FixedUpdate. See ApplyDriveInput.
        private DriveInput pendingRiderInput;
        ...
        // feels stuck — see RigidbodyMotor.ApplyDriveInput for the full reasoning.
        ...
        private void DriveFromRider(in DriveInput input, float deltaTime)
        {
            if (!body || arcing)
                return;
        ...
        private void RotateToward(Vector3 direction, float deltaTime)
        {
            ...
            // than transform.rotation for the interpolation reason in ApplyDriveInput.
