...
namespace SpaceGame.Agents
{
    ...
    public class AgentTargeting : MonoBehaviour
    {
        ...
        // Both are floored at proximityAcquireRange, so a narrow profile can never shrink the
        // working range inside the point-blank bypass — an agent that fails to react to something
        // it is standing next to reads as broken AI.
        /// <summary>Acquisition range. What the agent can actually spot at.</summary>
        public float SightRange =>
            Mathf.Max(effectiveAcquisitionRange, settings.proximityAcquireRange);

        /// <summary>Retention range, so a target that drifts out of reach is dropped too.</summary>
        public float LoseRange =>
            Mathf.Max(effectiveLoseRange, settings.proximityAcquireRange);
        ...
        private void Reevaluate()
        {
            ...
            // Sight range, not the raw acquisition range, so the point-blank floor applies here too.
