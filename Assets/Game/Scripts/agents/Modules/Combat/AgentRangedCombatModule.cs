...
namespace SpaceGame.Agents
{
    public class AgentRangedCombatModule : BehaviourModuleBase, IFacingModule
    {
        ...
        private void FireOne(Transform target)
        {
            ...
            PresentShot(muzzle.position, spawnPos, aimDir);
        }

        /// <summary>
        /// Put one shot in the air, with its report and its animation.
        ///
        /// <para>
        /// The Present half of the same split <see cref="SpaceGame.Items.UsableItem"/> draws
        /// between Use and Present: everything that decides — the target, the lead, the spread —
        /// happens above this line and never below it.
        /// </para>
        /// <para>
        ...
        /// </para>
        /// </summary>
        private void PresentShot(Vector3 reportPosition, Vector3 spawnPos, Vector3 aimDir)
        {
            AgentWeaponDefinition activeWeapon = ActiveWeapon;
            if (activeWeapon == null || activeWeapon.projectilePrefab == null)
                return;
            ...
            // designer hangs one, which is what makes it presentation: it is handed the barrel
            // position, where OnMiss and OnKillEvent are handed the outcome of the shot.
