...
namespace SpaceGame.Agents
{
    ...
    public class AgentProjectile : MonoBehaviour
    {
        ...
        private void OnCollisionEnter(Collision collision)
        {
            ...
            {
                // Through Damage rather than straight at the HealthComponent, so an AI's shot goes
                // through the same path a player's does — the one that records who fired it.
                Damage.Apply((damageable as Component)?.gameObject, damage, shooterTransform);

                // The callback is what drives the shooter's OnMiss/OnKill events.
