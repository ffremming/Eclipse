...
// the locomotion is the thing deciding where the machine goes, and wrong whenever something else
// places the body — the locomotion runs in LateUpdate, so it overwrites that pose every frame and
// the body never moves at all.
//
// These tests pin the following mode down from the outside: the transform is written by the test,
// and the machine has to leave it alone while still stepping its feet against the ground it is
// being carried over.
    ...
    public class ExternallyPosedLocomotionTests
    {
        ...
        public void AnExternallyPosedMachineNeverWritesItsOwnBody()
        {
            ...
            Assert.AreEqual(parked, m.transform.position,
                "an externally posed machine moved itself; this is what overwrites the pose it was " +
                "given every frame and strands the body where it started");
        ...
        public void AnExternallyPosedMachineStepsAgainstMotionItDidNotChoose()
        {
            ...
            // What an external poser does: write the pose, then let everything else read it. No
            // SetTwist anywhere — nothing is commanding this machine, so measured motion is the
            // only thing the gait has to go on.
        ...
        public void FollowingDoesNotFightThePoseItIsGiven()
        {
            ...
            Assert.AreEqual(0, m.LastFrame.UnreachableLegs,
                "the legs never recovered from a teleport; a machine placed from outside would " +
                "arrive at every jump stuck in the splits");
        }
        ...
        public void TakingTheBodyBackResumesFromWhereItActuallyIs()
        {
            ...
            // whatever placed the body does when it hands it back.
            m.ExternallyPosed = true;
