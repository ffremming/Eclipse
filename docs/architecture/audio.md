...
- **Sound belongs on the path that actually happened.** `HealthComponent.OnRevive` and the like are
  raised once per genuine event, so a sound hung there cannot fire on a half-completed action.
